...
grandpa_img = pygame.image.load('sprites/grandma_img.png')
farm_img = pygame.image.load('sprites/farm_img.jpg')
mine_img = pygame.image.load('sprites/mine_img.jpg')
factory_img = pygame.image.load('sprites/factory_img.jpeg')
bank_img = pygame.image.load('sprites/bank_img.jpg')
temple_img = pygame.image.load('sprites/temple_img.jpg')
...
class CookieDisplayObj:
    ...
    def draw(self, score, user_cps):
        ...
        score = font.render('{} cookies'.format(int( format_number(score) )), True, WHITE)
...
class Building:
    ...
    def drawDisplayBox(self):
        ...
        production = description_font.render('Each {} produces {:.1f} cupcakes per second'.format(self.name, self.cps), True, WHITE)
        quantity = description_font.render('You have {} {}s producing {:.1f} cupcakes per second'.format(self.quantity, self.name, self.cps * self.quantity), True, WHITE)
        created = description_font.render('{}s have created {} cupcakes so far'.format(self.name, math.floor(self.created)), True, WHITE)
...
class Player:
    ...
    def updateTotalCPS(self, list_of_buildings):
        ...
        '''Updates number of falling cupcakes'''
        timer = 0
...
'''Buildings'''
store_y = 212
cursor = Building('Cursor', 700, store_y, cursor_img, cursor_icon, base_cost=15, increase_per_purchase=1.15, cps=1)
grandma = Building('Grandpa', 700, store_y + 64*1, grandpa_img, grandma_icon, base_cost=100, increase_per_purchase=1.15, cps=5)
farm = Building('Giant oven', 700, store_y + 64*2, farm_img, farm_icon, base_cost=1100, increase_per_purchase=1.15, cps=20)
mine = Building('Mine', 700, store_y + 64*3, mine_img, mine_icon, base_cost=12000, increase_per_purchase=1.15, cps=50)
factory = Building('Factory', 700, store_y + 64*4, factory_img, factory_icon, base_cost=130000, increase_per_purchase=1.15, cps=300)
bank = Building('Stonk', 700, store_y + 64*5, bank_img, factory_icon, base_cost=1400000, increase_per_purchase=1.15, cps=1500)
temple = Building('Temple', 700, store_y + 64*6, temple_img, temple_icon, base_cost=20000000, increase_per_purchase=1.15, cps=7800)
wizard_tower = Building('Omnipresent loafs', 700, store_y + 64*7, wizard_tower_img, wizard_tower_icon, base_cost=330000000, increase_per_purchase=1.15, cps=30000)
